  ...
  def PDE(self, score_fn, 
      ...
      class score_FP(self.__class__):
          ...
          def fp(self, x, t):
            ...
            def normlization(xxx):
                  mean = torch.mean(xxx)
                  std1 = torch.std(xxx)
                  normalized_x = torch.div(torch.sub(xxx, mean), std1)
                  return normalized_x
            ...
            if self.scalar_fp in ("True"):
                ...
                f_dot_s = torch.einsum('bs,bs->b', x, s)
                RHS = (g_pow/2) * ( div_s + s_l22 + f_dot_s )  
                # print("div_s", torch.linalg.norm(div_s, ord=2))
                # print("s_l22", torch.linalg.norm(s_l22, ord=2))
                # print("f_dot_s", torch.linalg.norm(f_dot_s, ord=2))
                # RHS = normlization(RHS)
                D = x.shape[-1]
                if self.list_dim == []:        
                    self.list_dim = range(D)
                ...
                # RHS = (g_pow/2) * diff.gradient( div_s + s_l22 + f_dot_s, x)  
                RHS = (g_pow.reshape(-1, 1)/2) * diff.gradient(div_s + s_l22 + f_dot_s, x)     
                if torch.isnan(RHS).any() != False:
                    print("RHS", RHS)

                "Computing LHS"
                # dsdt = diff.t_finite_diff(self.flat_model, x, t)
                # if torch.isnan(dsdt).any() != False:
                #     print("dsdt", dsdt)
                # error = (dsdt - RHS)
                # res = torch.linalg.norm(error, ord=2, dim=1)  
                # if torch.isnan(res).any() != False:
                #    print("res1", res)
                ...
                if self.normalize:
                    res = res/(D ** self.m)
                    scalar_res = scalar_res/(D)
                    if torch.isnan(res).any() != False:
                      print("res4", res)             
                vec_res = res                 
                print('scalar_res', scalar_res)
                if torch.isnan(vec_res).any() != False:
                   torch.save(vec_res, 'vec_res.pt')
                # assert torch.isnan(vec_res).any() == False, "Tensor contains NaN values"
...
class VESDE(SDE):
  ...
  def discretize(self, x, t):
    ...
    adjacent_sigma = torch.where(timestep == 0, torch.zeros_like(t),
         self.discrete_sigmas[timestep.to(self.discrete_sigmas.device) - 1].to(t.device))
  ...
  def PDE(self, score_fn, 
      ...
      class score_FP(self.__class__):
          ...
          def fp(self, x, t):
            ...
            #t大于0.5小于等于1的位置，mask为1
            mask[(t >= 1e-3) & (t <= 0.50)] = 1
            s = self.flat_model(x, t)                 
            if self.div_est == 'exact':
                div_s = diff.batch_div(self.flat_model, x, t)
            elif self.div_est == 'approx':    
            ...
            if self.scalar_fp in ("True"):
                ...
                # print("RHS", RHS)
                # print("RHS", RHS * mask)
                scalar_res = torch.linalg.norm(RHS * mask, ord=2) if self.train else np.linalg.norm(RHS, ord=2)       
                print("scalar_fpe", scalar_res)
                g_pow = (diffusion[:, None])**2 
                D = x.shape[-1]
                if self.list_dim == []:        
                    self.list_dim = range(D)
                ...
                "Computing LHS"
                dsdt = diff.t_finite_diff(self.flat_model, x, t)
                # print(torch.linalg.norm((dsdt - RHS), ord=2, dim=1))
                res = torch.linalg.norm((dsdt - RHS), ord=2, dim=1) * mask
                # print("res", res)
                res = res ** self.m
                if self.wgt == 'convention':
                    res = diffusion * res
